...
// Importe une fonction pour getMatieresByClassAndTeacher si tu as la liaison
...
export default function EvaluationClassForm({ onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    if (!possibleTeachers.some(t => t.id === selectedTeacherId)) {
      setSelectedTeacherId("");
    }
  ...
  const handleSaveAll = async () => {
    ...
    const validNotes = draftEvaluations.filter(d => d.note.trim() !== "" && !isNaN(Number(d.note)));
