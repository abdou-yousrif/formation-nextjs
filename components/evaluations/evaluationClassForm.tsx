...
import { getElevesByClass } from "@/lib/supabase/evaluations";        // ← mis à jour
import { getMatieresByClass } from "@/lib/supabase/evaluations";
...
import { Classe, getClasses } from "@/lib/supabase/classes";
import { getElevesByClass } from "@/lib/supabase/evaluations";        // ← mis à jour
import { getMatieresByClass } from "@/lib/supabase/evaluations";      // ← nouvelle fonction
import { Teacher, getTeachers } from "@/lib/supabase/teachers";
import { DraftEvaluation, addBulkEvaluations } from "@/lib/supabase/evaluations";
...
export default function EvaluationClassForm({ onClose, onSuccess }: Props) {
  ...
  const [classes, setClasses] = useState<Classe[]>([]);
  const [matieres, setMatieres] = useState<any[]>([]);           // matières de la classe
  const [teachers, setTeachers] = useState<Teacher[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error(err);
        setErrorMsg("Erreur de chargement des données initiales");
      }
    };
    fetchData();
  }, []);

  // Quand la classe change → charger élèves + matières de cette classe
  useEffect(() => {
    if (!selectedClassId) {
      setDraftEvaluations([]);
      setMatieres([]);
    ...
    const loadData = async () => {
      try {
        const [eleves, matieresClass] = await Promise.all([
          getElevesByClass(selectedClassId),
          getMatieresByClass(Number(selectedClassId))
        ]);

        setMatieres(matieresClass);

        const draft = eleves.map((e: any) => ({
          eleve_id: e.id,
          eleve: { first_name: e.first_name, last_name: e.last_name },
          note: "",
          commentaire: "",
        ...
        setDraftEvaluations(draft);
        setSelectedMatiereId("");     // reset matière
        setSelectedTeacherId("");
      } catch (err) {
        console.error(err);
  ...
  // Reset professeur quand matière change
  useEffect(() => {
    setSelectedTeacherId("");
  }, [selectedMatiereId]);
  ...
  const handleSaveAll = async () => {
    ...
    if (!selectedClassId || !selectedMatiereId || !selectedTeacherId || !selectedType.trim()) {
      setErrorMsg("Veuillez remplir tous les champs obligatoires (classe, matière, professeur, type)");
      return;
    }
    ...
    if (validNotes.length === 0) {
      setErrorMsg("Aucune note valide saisie (entre 0 et 20)");
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(err?.message || "Erreur lors de l'enregistrement des notes");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[96vh] flex flex-col overflow-hidden">
        ...
        <div className="p-5 border-b bg-gradient-to-r from-blue-50 to-indigo-50 flex justify-between items-center">
          <h2 className="text-2xl font-bold text-gray-800">Saisie des notes par classe</h2>
          <Button variant="ghost" size="icon" onClick={onClose}>
            <X className="h-6 w-6" />
          </Button>
        </div>
        ...
        {/* Filtres */}
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 border-b">
          <div>
            <Label>Classe</Label>
            <select
              value={selectedClassId}
          ...
          <div>
            ...
            <select
              ...
              onChange={e => setSelectedMatiereId(e.target.value)}
              disabled={!selectedClassId}
              className="w-full border rounded-lg px-4 py-2.5 mt-1 disabled:opacity-50"
            >
              <option value="">Choisir une matière</option>
              {matieres.map(m => (
                <option key={m.id} value={m.id}>
        ...
        <div className="flex-1 overflow-auto p-6 bg-gray-50">
          {draftEvaluations.length > 0 ? (
            <table className="w-full border-collapse min-w-max">
              <thead className="sticky top-0 bg-white shadow-sm z-10">
                <tr>
                  ...
                  <th className="p-4 text-center w-28">Note (/20)</th>
              ...
              <tbody>
                {draftEvaluations.map((draft, idx) => (
                  <tr key={draft.eleve_id} className="border-b hover:bg-blue-50/30">
                    <td className="p-4 font-medium">
                      {draft.eleve.first_name} {draft.eleve.last_name}
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-center">
                      <Button variant="ghost" size="sm" onClick={() => removeFromDraft(idx)}>
            ...
            <div className="flex items-center justify-center h-full text-gray-500">
              {selectedClassId ? "Aucun élève dans cette classe" : "Sélectionnez une classe"}
            </div>
          )}
        </div>
        ...
        <div className="p-5 border-t bg-white flex justify-between items-center">
          ...
          <div className="flex gap-4">
            ...
            <Button 
              onClick={handleSaveAll}
              disabled={loading || filledCount === 0 || !selectedTeacherId || !selectedType.trim()}
              className="bg-indigo-600 hover:bg-indigo-700"
            >
              {loading ? "Enregistrement..." : `Enregistrer ${filledCount} note${filledCount > 1 ? 's' : ''}`}
              <Save className="ml-2 h-4 w-4" />
            </Button>
          </div>
        </div>
